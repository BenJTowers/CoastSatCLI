...
def initial_settings(config):
    ...
    filepath_data = config['output_dir']  
...
def batch_shoreline_detection(metadata, settings, inputs):
    ...
    settings['reference_shoreline'] = SDS_preprocess.get_reference_sl_from_geojson(settings['inputs']['reference_geojson'],settings['output_epsg'])
...
def apply_csv_tide_correction(settings, cross_distance, output):
    """
    Apply tide correction using CSV tide data and fixed beach slope.
    ...
    df = pd.DataFrame(out_dict)
    corrected_csv = os.path.join(filepath, 'transect_time_series_tidally_corrected.csv')
    ...
    # Plot raw vs tidally-corrected shoreline time-series
    fig = plt.figure(figsize=[15, 8], tight_layout=True)
    gs = gridspec.GridSpec(len(cross_distance), 1)
    gs.update(left=0.05, right=0.95, bottom=0.05, top=0.95, hspace=0.05)
    for i, key in enumerate(cross_distance):
        if np.all(np.isnan(cross_distance[key])):
            continue
        idx_nan = np.isnan(cross_distance[key])
        dates = [dates_sat[j] for j in np.where(~idx_nan)[0]]
        raw = cross_distance[key][~idx_nan]
        corrected = cross_distance_tidally_corrected[key][~idx_nan]
        ax = fig.add_subplot(gs[i, 0])
        ax.grid(linestyle=':', color='0.5')
        ax.plot(dates, raw, '-o', ms=6, mfc='w', label='Raw')
        ax.plot(dates, corrected, '-o', ms=6, mfc='w', label='Tidally Corrected')
        ax.set_ylabel('Distance [m]', fontsize=12)
        ax.text(0.5, 0.95, key, bbox=dict(boxstyle="square", ec='k', fc='w'),
                ha='center', va='top', transform=ax.transAxes, fontsize=14)
    ax.legend()
    fig.savefig(os.path.join(filepath, f'{sitename}_timeseries_corrected.jpg'), dpi=200)

    return cross_distance_tidally_corrected, tides_sat
...
def slope_estimation(settings, cross_distance, output):
    ...
    fes_yaml = settings['inputs']['fes_config']
...
def main(config_path):
    ...
    print(f'Starting site: {config["inputs"]["sitename"]}')

    inputs, settings, metadata = initial_settings(config)
    ...
    cross_distance_tidally_corrected, tides_sat = apply_csv_tide_correction(
        settings, cross_distance, output
    )
    slope_est = dict([])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Run CoastSat analysis using a settings.json'
    )
    parser.add_argument(
        '--config', '-c',
        required=True,
