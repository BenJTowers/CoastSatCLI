...
def get_reference_sl_from_geojson(ref_geojson_path, output_epsg):
    """
    Loads reference shoreline coordinates from a GeoJSON file and returns a list of coordinate arrays.
    Also saves a .pkl version for compatibility if one doesn't exist.

    Parameters:
    -----------
    ref_geojson_path : str
        Full path to the reference shoreline GeoJSON file.
    output_epsg : int
        EPSG code for the desired output projection.

    Returns:
    --------
    list of np.array : A list of shorelines as arrays of coordinates.
    """
    ref_geojson_path = Path(ref_geojson_path)
    ref_pkl_path = ref_geojson_path.with_suffix('.pkl')

    if ref_geojson_path.exists():
        print('Reference shorelines found in GeoJSON. Loading coordinates for each feature.')

        refsl_geojson = gpd.read_file(ref_geojson_path)
        refsl_geojson = refsl_geojson.to_crs(epsg=output_epsg)
        ...
        if not ref_pkl_path.exists():
            with open(ref_pkl_path, 'wb') as f:
                pickle.dump(ref_sl_list, f)
            print(f'Reference shorelines also saved as .pkl at {ref_pkl_path}')

        return ref_sl_list
    else:
        print("No GeoJSON reference shoreline found at", ref_geojson_path)
        return None
